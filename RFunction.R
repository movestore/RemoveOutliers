...
rFunction <- function(data, maxspeed=NULL, MBremove=TRUE, FUTUREremove=TRUE, accuracy_var=NULL, minaccuracy=NULL)
{
  ...
  if (is.null(maxspeed) & MBremove==FALSE & is.null(accuracy_var)) logger.info("No maximum speed provided, no accuracy variable defined and required to leave Movebank marked Outliers in. Return input data set.")
  ...
  if (!is.null(accuracy_var)) logger.info(paste("Remove positions with high location error:",accuracy_var,">",minaccuracy))
